...
function activate(context) {
    ...
    const openCommand = vscode.commands.registerCommand('devDocCopilot.open', () => {
        ...
        if (currentPanel) {
            ...
            currentPanel = vscode.window.createWebviewPanel('devDocCopilot', 'Dev-Doc Copilot', columnToShowIn || vscode.ViewColumn.One, {
                enableScripts: true,
            ...
            currentPanel.webview.onDidReceiveMessage(async (message) => {
                ...
                const project = config.get('project', 'vscode-project');
                switch (message.command) {
                    case 'chat':
                        try {
                            const response = await axios_1.default.post(`${backendUrl}/api/chat`, {
                                project: project,
                    ...
                    case 'getGraph':
                        try {
                            const response = await axios_1.default.post(`${backendUrl}/api/graph/data`, {
                                project: project
                            });
                            currentPanel?.webview.postMessage({
                                command: 'graphData',
